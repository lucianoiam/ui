...
static JSValue js_arc(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv)
{
   if (argc < 3)
      return JS_EXCEPTION;
   double x, y, r;
   if (JS_ToFloat64(ctx, &x, argv[0]) || JS_ToFloat64(ctx, &y, argv[1]) || JS_ToFloat64(ctx, &r, argv[2]))
      return JS_EXCEPTION;
...
int main()
{
   ...
   std::function<lxb_dom_element_t*(lxb_dom_node_t*, const char*)> find_element_by_id =
   ...
   lxb_dom_element_t* container =
       find_element_by_id((lxb_dom_node_t*)lxb_html_document_body_element(document), "container");
   if (!container) {
