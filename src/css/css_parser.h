...
namespace css {
struct Decls { std::unordered_map<std::string,std::string> kv; };
Decls parse_inline(const std::string& cssText);
struct FlexShorthand { bool haveGrow=false; float grow=0; bool haveShrink=false; float shrink=1; bool basisAuto=false; bool basisPercent=false; bool basisPoint=false; float basisValue=0; };
