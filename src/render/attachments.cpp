...
DomElementRenderData* ensure_render_data(dom::Element* el){
    if(!el) return nullptr; auto it=g_renderData.find(el); if(it==g_renderData.end()){
...
DomElementRenderData* get_render_data(dom::Element* el){
    if(!el) return nullptr; auto it=g_renderData.find(el); if(it==g_renderData.end()) return nullptr; return it->second.get(); }
void free_render_data(dom::Element* el){
    if(!el) return; auto it=g_renderData.find(el); if(it!=g_renderData.end()) {
        if(it->second->yogaNode){ YGNodeRef n=(YGNodeRef)it->second->yogaNode; YGNodeFreeRecursive(n); it->second->yogaNode=nullptr; }
