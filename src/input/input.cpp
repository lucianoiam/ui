...
void init(std::shared_ptr<dom::Document> doc) { g_doc = doc; }
void shutdown() { g_doc.reset(); }
...
static void collectElementsWith(dom::Element* root, std::vector<dom::Element*>& out) {
    ...
    for (auto &c : root->childNodes) {
        if (c && c->nodeType == dom::NodeType::ELEMENT) collectElementsWith((dom::Element*)c.get(), out);
    }
}

dom::Element* hitTest(int x, int y) {
    auto doc = g_doc.lock(); if(!doc) return nullptr;
    // breadth-first collection of all elements
    std::vector<dom::Element*> els;
    for (auto &c : doc->childNodes) if (c && c->nodeType == dom::NodeType::ELEMENT) collectElementsWith((dom::Element*)c.get(), els);
    ...
    for (auto it = els.rbegin(); it != els.rend(); ++it) {
        ...
        if (itAttr == el->attributes.end()) continue;
        const std::string& s = itAttr->second;
        int left = parseDecl(s, "left");
        int top = parseDecl(s, "top");
        int w = parseDecl(s, "width"); if (w<0) w = 64;
        int h = parseDecl(s, "height"); if (h<0) h = 64;
        if (x>=left && x<=left+w && y>=top && y<=top+h) return el;
    }
    return nullptr;
}
