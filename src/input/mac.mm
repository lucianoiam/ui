...
extern void present_surface(NSImageView *iv, sk_sp<SkSurface> surface, int W,
                            int H);
...
- (void)dispatchMouseEventType:(const char *)type x:(int)x y:(int)y {
  ...
  JSValue fn =
      JS_GetPropertyStr(g_deferred_ctx, global, "__dispatchNativeMouse");
  if (JS_IsFunction(g_deferred_ctx, fn)) {
    JSValue obj = JS_NewObject(g_deferred_ctx);
    JS_SetPropertyStr(g_deferred_ctx, obj, "type",
                      JS_NewString(g_deferred_ctx, type));
    JS_SetPropertyStr(g_deferred_ctx, obj, "clientX",
                      JS_NewInt32(g_deferred_ctx, x));
    JS_SetPropertyStr(g_deferred_ctx, obj, "clientY",
                      JS_NewInt32(g_deferred_ctx, y));
    JSValue args[1] = {obj};
