...
function ShapeCell({index}) {
   ...
   useLayoutEffect(() => {
      ...
      function onDown(e) {
         ...
         const mL = /left:(\d+)/.exec(st);
         const mT = /top:(\d+)/.exec(st);
         const curL = mL ? +mL[1] : 0;
         const curT = mT ? +mT[1] : 0;
         dragging = true;
         offX = (e.clientX | 0) - curL;
         offY = (e.clientY | 0) - curT;
      }
      function onMove(e) {
         if (!dragging) return;
         const x = e.clientX | 0;
         const y = e.clientY | 0;
         const nx = Math.max(0, Math.min(800 - size, x - offX));
         const ny = Math.max(0, Math.min(600 - size, y - offY));
         el.setAttribute('style', styleFor(nx, ny));
