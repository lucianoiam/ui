...
static void ctx2d_finalizer(JSRuntime *rt, JSValue val) {
  auto *state =
      static_cast<CanvasContextState *>(JS_GetOpaque(val, g_ctx2d_class_id));
  delete state;
}
static JSValue js_arc(JSContext *ctx, JSValueConst this_val, int argc,
                      JSValueConst *argv);
static JSValue js_fill(JSContext *ctx, JSValueConst this_val, int argc,
                       JSValueConst *argv);
static JSValue js_getContext(JSContext *ctx, JSValueConst this_val, int argc,
                             JSValueConst *argv);
static JSValue js_getCanvasById(JSContext *ctx, JSValueConst this_val, int argc,
                                JSValueConst *argv);

static CanvasContextState *get_ctx_state(JSContext *ctx,
                                         JSValueConst this_val) {
  return static_cast<CanvasContextState *>(
      JS_GetOpaque2(ctx, this_val, g_ctx2d_class_id));
}
...
static JSValue js_fill(JSContext *ctx, JSValueConst this_val, int argc,
                       JSValueConst *argv) {
  auto *state = get_ctx_state(ctx, this_val);
  if (!state || !g_skiaCanvas || !state->hasArc)
    return JS_UNDEFINED;
  SkPaint paint;
  paint.setAntiAlias(true);
  paint.setColor(state->fillColor);
  paint.setStyle(SkPaint::kFill_Style);
  g_skiaCanvas->drawCircle((float)state->arcX, (float)state->arcY,
                           (float)state->arcR, paint);
...
static SkColor parse_color(const char *s) {
  ...
  if (s[0] == '#') {
    ...
    if (len == 6) {
      ...
      r = (unsigned)(isdigit(rch) ? (rch - '0') : 0);
      r = r * 16 + r;
      g = (unsigned)(isdigit(gch) ? (gch - '0') : 0);
      g = g * 16 + g;
      b = (unsigned)(isdigit(bch) ? (bch - '0') : 0);
      b = b * 16 + b;
      return SkColorSetRGB(r, g, b);
    }
  }
...
static JSValue js_fillStyle_get(JSContext *ctx, JSValueConst this_val, int argc,
                                JSValueConst *argv) {
  auto *state = get_ctx_state(ctx, this_val);
  if (!state)
    return JS_UNDEFINED;
...
static JSValue js_fillStyle_set(JSContext *ctx, JSValueConst this_val, int argc,
                                JSValueConst *argv) {
  if (argc < 1)
    return JS_UNDEFINED;
  auto *state = get_ctx_state(ctx, this_val);
...
static JSValue js_getContext(JSContext *ctx, JSValueConst this_val, int argc,
  ...
  if (strcmp(ctxType, "2d") == 0) {
    ...
    JS_SetPropertyStr(ctx, ctx2d, "arc",
                      JS_NewCFunction(ctx, js_arc, "arc", 3));
    JS_SetPropertyStr(ctx, ctx2d, "fill",
    ...
    JS_DefinePropertyGetSet(ctx, ctx2d, JS_NewAtom(ctx, "fillStyle"), getter,
...
static JSValue js_getCanvasById(JSContext *ctx, JSValueConst this_val, int argc,
  ...
  if (strcmp(id, "mycanvas") == 0) {
    ...
    JS_SetPropertyStr(ctx, canvasObj, "getContext",
                      JS_NewCFunction(ctx, js_getContext, "getContext", 1));
  }
...
int main() {
  ...
  lxb_status_t status = lxb_html_document_parse(
      document, (const lxb_char_t *)html_content, strlen(html_content));
  ...
  std::function<lxb_dom_element_t *(lxb_dom_node_t *, const char *)>
      find_element_by_id =
          [&](lxb_dom_node_t *node, const char *id) -> lxb_dom_element_t * {
    if (node->type == LXB_DOM_NODE_TYPE_ELEMENT) {
      lxb_dom_element_t *el = (lxb_dom_element_t *)node;
      size_t attr_len = 0;
      const lxb_char_t *attr_val = lxb_dom_element_get_attribute(
          el, (const lxb_char_t *)"id", 2, &attr_len);
      if (attr_val != nullptr) {
        if (attr_len == strlen(id) &&
            strncmp((const char *)attr_val, id, attr_len) == 0) {
          return el;
        }
      }
    }
  ...
  lxb_dom_element_t *container = find_element_by_id(
      (lxb_dom_node_t *)lxb_html_document_body_element(document), "container");
  if (!container) {
  ...
  for (lxb_dom_node_t *child = ((lxb_dom_node_t *)container)->first_child;
       child; child = lxb_dom_node_next(child)) {
    if (child->type == LXB_DOM_NODE_TYPE_ELEMENT) {
      ...
      size_t style_len = 0;
      const lxb_char_t *style_val = lxb_dom_element_get_attribute(
  ...
  JSValue getCanvasById =
      JS_NewCFunction(ctx, js_getCanvasById, "getCanvasById", 1);
