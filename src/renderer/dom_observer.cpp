...
void dom_notify_element_created(dom::Element* el) {
    for (auto* o : g_observers) o->onElementCreated(el);
}
void dom_notify_element_removed(dom::Element* el) {
    for (auto* o : g_observers) o->onElementRemoved(el);
}
void dom_notify_attribute_changed(dom::Element* el, const std::string& name, const std::string& oldValue, const std::string& newValue) {
    for (auto* o : g_observers) o->onAttributeChanged(el, name, oldValue, newValue);
}
void dom_notify_childlist_changed(dom::Element* el) {
    for (auto* o : g_observers) o->onChildListChanged(el);
}
